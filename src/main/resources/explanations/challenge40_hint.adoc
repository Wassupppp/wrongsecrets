...
- Copy the value of `key` from `secrchallenge.json` and type in `echo -n "REDACTED" | xxd -p`. This gives you the decryption key in hexadecimal format.
- Copy the value of `secret` from `secrchallenge.json`.
- Then, use the obtained decryption key to decrypt the by typing `echo "REDACTED" | openssl enc -a -d -aes-128-ecb -K 33556c79666d6667726f36756831634e -out decrypted.md`
